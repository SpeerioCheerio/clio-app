...
class VersionControl {
    ...
    renderProjectGroups() {
        ...
        this.groupedProjects.forEach(group => {
            ...
            // Toggle collapse
            groupEl.querySelector('.project-group-header').addEventListener('click', (e) => {
                // Prevent project click event
                if (e.target.closest('.project-tab')) return;
                
                groupEl.querySelector('.project-group-header').classList.toggle('collapsed');
                groupEl.querySelector('.project-group-content').classList.toggle('collapsed');
            });
        });
    }
    ...
    async loadVersions(projectName) {
        try {
            document.getElementById('versionListHeader').style.display = 'block';
            document.getElementById('versionListHeader').textContent = `Versions for: ${projectName}`;
        ...
        if (confirmation) {
            try {
                ...
                if (response.success) {
                    ...
                    if (this.currentProject === projectName) {
                        this.currentProject = null;
                        document.getElementById('versionList').innerHTML = '<div class="empty-state"><p>Project deleted</p></div>';
                    }
                    await this.loadProjects();
                } else {
